...
rangeRead.read = ( uri, opt = {} ) => {
    ...
    opt.buffer_size = opt.buffer_size_alpha || null;
    ...
    .then((stat) => {
        ...
        if(opt.buffer_size_alpha){
            opt.buffer_size = Math.round( file.stat.size * opt.buffer_size_alpha );
        }

        file.buffer = new Buffer.alloc( opt.buffer_size );
