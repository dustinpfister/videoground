...
const DEFAULT_OPTIONS = {
    dir_userdata: dir_userdata,
    uri_data: path.join( DEFAULT_DIR_USERDATA, 'data.json')
    data_default: {
...
const createUserDataFolder = function(opt){
    return new Promise((resolve, reject) => {
        fs.mkdir(opt.dir_userdata, { recursive: true }, (err) => {
            if (err){
                reject(err);
            }else{
...
const createUserDataFile = function(opt){
    return readFile(opt.uri_data, 'utf8')
    .then(()=>{
    ...
    .catch((e)=>{
        ...
        if(e.code === 'ENOENT'){
            ...
            return writeFile(opt.uri_data, JSON.stringify(opt.data_default), 'utf8' )
        }
...
api.set = (opt, key, value) => {
    ...
    .then((obj)=>{
        ...
        obj[key] = value;
        return writeFile( opt.uri_data, JSON.stringify(obj) );
    });
};
